...
class CartControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        /* JJ prus */
        $index = ConfigurationCore::get('WD_CUSTOMIZATION_INDEX');
        $custom_prices = Db::getInstance()->executeS('
        SELECT cu.`id_customization`, cd.`index`, cd.`value`, cd.`type`, cu.`id_product`, cd.`dimension_price` FROM `' . pSQL(_DB_PREFIX_) . 'customization` cu
        ...
        AND cd.index = ' . (int) $index . ' AND cd.type = 1 AND in_cart = 1 order by cu.id_customization asc');
        /* JJ prus */
    ...
    public function displayAjaxUpdate()
    {
        ...
        if (!$this->errors) {
            ...
            $sql = 'select id_customization from ' . pSQL(_DB_PREFIX_) . 'customization where id_product = ' . (int) $this->id_product . ' and id_product_attribute=' . $this->id_product_attribute . ' and id_cart=' . (int) $this->context->cart->id;
            $customization = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sql);

            $customization_id = is_array($customization) && isset($customization['id_customization']) ? (int) $customization['id_customization'] : false;
    ...
    protected function processChangeProductInCart()
    {
        ...
        /* JJ prus Modification */
        if ($mode == 'update' && $this->customization_id !== 0) {
            $this->{$ErrorKey}[] = $this->trans(
                'You can not update customizable product in cart. The price is calculated based on the dimension entered. Please delete and add new to update the quantity.',
                array(),
                'Shop.Notifications.Error'
            );
            return;
        ...
        if (!$this->errors) {
            ...
            if (!$this->errors) {
                ...
                if (Tools::getValue('discretion') == 'on') {
                    setcookie('cart[' . $this->id_product . ']', $this->dim);
                }
                /* JJ prus Modification */
                ...
                $sql = 'select id_customization from ' . pSQL(_DB_PREFIX_) . 'customization where id_product = ' . (int) $this->id_product . ' and id_cart=' . (int) $this->context->cart->id;
                $customization = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sql);

                $customization_id = is_array($customization) && isset($customization['id_customization']) ? (int) $customization['id_customization'] : false;

                if (!$customization_id) {
                    $product = new Product($this->id_product, false, false);
                    if ($product->customizable) {
                        $this->{$ErrorKey}[] = $this->trans(
                            'No customization id created.',
                            array(),
                            'Shop.Notifications.Error'
                        );
                        return;
